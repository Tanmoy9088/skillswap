...
export default function SkillDetailsPage() {
      ...
      <main className="min-h-screen bg-[#F7F7FF] px-6 py-20">
        <div className="mx-auto max-w-7xl animate-pulse">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((item) => (
              <div key={item} className="h-72 rounded-2xl bg-gray-200" />
            ))}
          </div>
        </div>
      </main>
  ...
  if (!data?.skill) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F7F7FF] px-6">
        <div className="max-w-lg rounded-2xl bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-bold text-[#17366F]">Skill not found</h1>

          <p className="mt-3 text-gray-500">No offered skill was found for:</p>

          <p className="mt-2 font-semibold text-indigo-600">{skillName}</p>
        </div>
      </main>
    );
  }
