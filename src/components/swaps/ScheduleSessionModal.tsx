...
const ScheduleSessionModal = ({
  ...
  const selectedSessionOption = useMemo(
    () =>
      sessionOptions.find((option) => option.id === selectedSessionOptionId),
  ...
  const isTimeWithinAvailability = (selectedTime: string) => {
    if (
      !selectedTime ||
  ...
  const availabilityText = useMemo(() => {
    ...
    if (selectedAvailability.length === 0) {
      return `Mentor is not available on ${DAYS[new Date(`${date}T00:00:00+05:30`).getDay()]}.`;
    }
  ...
  const formattedDate = date
    ? new Date(`${date}T00:00:00+05:30`).toLocaleDateString()
    : "";
        ...
        {!isConfirming && (
          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            ...
            <div className="flex gap-3">
              ...
              >
                {" "}
                <Calendar className="h-4 w-4" /> Review{" "}
              </button>{" "}
            </div>{" "}
          </form>
        )}{" "}
